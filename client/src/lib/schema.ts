...
export const convertToSchema = (nodes: TreeNode[]): Record<string, unknown> => {
  const convert = (node: TreeNode): unknown => {
    if (isArrayNode(node)) {
      if (node.arrayItemType === "object" && node.children) {
        const childSchema = node.children.reduce((acc, child) => {
  ...
  const schema = rootNodes.reduce((acc, node) => {
